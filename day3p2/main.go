...
func closestIntersectionToOrigin(wire1, wire2 []segment) int {
	...
	for _, s1 := range wire1 {
		...
		for _, s2 := range wire2 {
			...
			if err == nil {
				d := lengthToPoint(s1, pt) + lengthToPoint(s2, pt) + seg1Len + seg2Len
				if d < dist {
					dist = d
				}
			}
...
func length(s segment) int {
	return abs(s.from.x-s.to.x) + abs(s.from.y-s.to.y)
}

func lengthToPoint(s segment, pt point) int {
	if s.from.y == s.to.y { // s is horizontal
		return abs(s.from.x - pt.x)
	}

	return abs(s.from.y - pt.y)
}
...
func intersection(s1, s2 segment) (point, error) {
	...
	switch {
	case s1.from.y == s1.to.y: // s1 is horizontal
		horiz = s1
		...
		verti = s2
	case s2.from.y == s2.to.y: // s2 is horizontal
		horiz = s2
